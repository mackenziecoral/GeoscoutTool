...
apikey <- "c1273660-f4c2-11ef-82f9-3195861b8d44" # IMPORTANT: Replace with your actual Wood Mackenzie API key
        ...
        tabPanel("Production Analysis",
                 ...
                 tabsetPanel(
                   ...
                   tabPanel("Filtered Group Cumulative",
                            ...
                            hr(),
                            # --- Start Comparison UI ---
                            h4("API Data Comparison"),
                            materialSwitch(inputId = "toggle_api_comparison", label = "Enable API Data Comparison", status = "primary", right = TRUE),
                            conditionalPanel(
                              condition = "input.toggle_api_comparison == true",
                              wellPanel(
                                h5("API Data Selection (Independent of Main Filters)"),
                                p("Select companies and assets from the Wood Mackenzie API dataset to compare against the filtered SQL data."),
                                fluidRow(
                                  column(6, pickerInput("api_company_filter", "Select Company:", choices = c("Loading..." = ""), multiple = TRUE, options = pickerOptions(actionsBox = TRUE, liveSearch = TRUE, noneSelectedText = "Select Company..."))),
                                  column(6, pickerInput("api_asset_filter", "Select Asset (Field):", choices = c("Loading..." = ""), multiple = TRUE, options = pickerOptions(actionsBox = TRUE, liveSearch = TRUE, noneSelectedText = "Select Asset...")))
                                ),
                                actionButton("fetch_api_data", "Fetch & Compare API Data", class = "btn-success btn-block"),
                                hr(),
                                h5("Comparison: Total Daily BOE Rate (kboe/d)"),
                                plotOutput("comparison_plot", height = "50vh")
                              )
                            ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$reset_filters, {
    ...
    # Reset API comparison filters as well
    updateMaterialSwitch(session, "toggle_api_comparison", value = FALSE)
    updatePickerInput(session, "api_company_filter", selected = character(0))
    updatePickerInput(session, "api_asset_filter", selected = character(0))
    reactive_vals$api_comparison_data <- NULL
  ...
  # --- API Data Comparison Logic ---
  
  # A. Observe the toggle switch to fetch API data for filter choices
  observeEvent(input$toggle_api_comparison, {
    # Only fetch if the switch is ON and data hasn't been fetched yet
    if (input$toggle_api_comparison == TRUE && is.null(reactive_vals$api_base_data)) {
      
      showNotification("Fetching API data for filters...", type = "message", id = "apiFetchMsg", duration = NULL)
      
      api_url <- "https://data.woodmac.com/query/upstream_weekly/imp-met/odata/field_company_net_annual_production_kbd?$filter=country_name eq 'Canada'"
      ...
      tryCatch({
        ...
        if (httr::status_code(response) == 200) {
          ...
          data_list <- jsonlite::fromJSON(content_raw)
          api_data_dt <- as.data.table(data_list$value)
          
          # Store the fetched data
          reactive_vals$api_base_data <- api_data_dt
          
          # Populate filters
          company_choices <- sort(unique(api_data_dt$company_name))
          asset_choices <- sort(unique(api_data_dt$field_name))
          
          updatePickerInput(session, "api_company_filter", choices = company_choices, selected = NULL)
          updatePickerInput(session, "api_asset_filter", choices = asset_choices, selected = NULL)
          
          removeNotification("apiFetchMsg")
          showNotification("API filter choices loaded.", type = "message", duration = 3)
        } else {
          error_msg <- paste("API Error:", httr::status_code(response), "-", httr::content(response, "text"))
          showNotification(error_msg, type = "error", duration = 10)
          removeNotification("apiFetchMsg")
        }
      }, error = function(e) {
        showNotification(paste("Failed to connect to API:", e$message), type = "error", duration = 10)
        removeNotification("apiFetchMsg")
      })
    }
  })
  
  # B. Process data and create comparison dataset when the button is clicked
  comparison_data <- eventReactive(input$fetch_api_data, {
    req(input$toggle_api_comparison == TRUE)
    showNotification("Processing comparison data...", type = "message", id = "compProcMsg", duration = NULL)
    
    # 1. Process API Data
    api_data_processed <- data.table()
    if (!is.null(reactive_vals$api_base_data) && !is.null(input$api_company_filter) && !is.null(input$api_asset_filter)) {
      
      filtered_api <- reactive_vals$api_base_data[
        company_name %in% input$api_company_filter & field_name %in% input$api_asset_filter
      ]
      
      if (nrow(filtered_api) > 0) {
        api_data_processed <- filtered_api[unit == 'kboe/d', .(
          Value = sum(as.numeric(metric_value), na.rm = TRUE)
        ), by = .(Year = year, Product = liquid_gas_breakdown)]
        api_data_processed[, DataSource := "API"]
      }
    }
    
    # 2. Process SQL Data (from filtered group)
    sql_data <- filtered_group_cumulative_data()$calendar_cumulative_data # This has daily BOE rate
    sql_data_processed <- data.table()
    if (!is.null(sql_data) && nrow(sql_data) > 0) {
      # This data is already aggregated by the breakout group. We sum it up across all groups for a total.
      sql_agg_by_year <- sql_data[, .(TotalBOE = sum(TotalMonthlyBOE_Cal, na.rm=TRUE)), by = .(Year = year(PROD_DATE))]
      
      # Simple approximation: Assume total BOE is the only category for SQL side for now
      # A full product breakdown would require re-querying or restructuring the SQL fetch logic significantly.
      sql_data_processed <- sql_agg_by_year[, .(
        Year,
        Product = "Total BOE",
        Value = TotalBOE / (1000 * 365.25), # Convert from boe/year to kboe/day
        DataSource = "SQL"
      )]
    }
    
    # 3. Combine
    if (nrow(api_data_processed) > 0 || nrow(sql_data_processed) > 0) {
      combined <- rbindlist(list(api_data_processed, sql_data_processed), use.names = TRUE, fill = TRUE)
      removeNotification("compProcMsg")
      return(combined)
    } else {
      removeNotification("compProcMsg")
      showNotification("No data found for comparison.", type = "warning", duration = 4)
      return(NULL)
    }
  })
  
  # C. Render the comparison plot
  output$comparison_plot <- renderPlot({
    plot_data <- comparison_data()
    
    if (is.null(plot_data) || nrow(plot_data) == 0) {
      return(ggplot() + labs(title = "No data available for comparison plot.", subtitle="Select API filters and click 'Fetch & Compare'.") + theme_void())
    }
    
    # Ensure Product is a factor for consistent coloring
    plot_data[, Product := factor(Product, levels = sort(unique(Product)))]
    
    ggplot(plot_data, aes(x = Year, y = Value, fill = Product)) +
      geom_area(alpha = 0.8) +
      facet_wrap(~ DataSource, scales = "free_y") +
      scale_y_continuous(labels = scales::comma) +
      scale_fill_viridis_d(option = "D") +
      labs(
        title = "Production Comparison: SQL Filtered Group vs. API Selection",
        subtitle = "Values are in thousand barrels of oil equivalent per day (kboe/d)",
        x = "Year",
        y = "Daily Production Rate (kboe/d)",
        fill = "Product Type"
      ) +
...
" and want to make the following changes to the selection:
* I want to update my `server` function to integrate with the Wood Mackenzie API for data comparison. The key steps are:
    1.  **Fetch API Data for Filters**: When the user enables the "API Data Comparison" toggle, I need to fetch the entire Canadian dataset from the specified URL. This data will be used to populate the "Company" and "Asset" dropdown filters. This should only happen once to avoid repeated API calls.
    2.  **Process and Compare Data**: When the user clicks the "Fetch & Compare API Data" button, I need to:
        a.  Filter the pre-fetched API data based on the user's company and asset selections.
        b.  Process this filtered API data to calculate the total daily BOE (in kboe/d) for each product type (`Oil`, `Gas`, `NGL`, `Condensate`) grouped by year.
        c.  Take the currently filtered SQL data from the `filtered_group_cumulative_data` reactive value.
        d.  Process the SQL data similarly, calculating total daily BOE for its products (`Oil`, `Condensate`, `Gas`) grouped by year, and converting the result to kboe/d to match the API data units.
        e.  Combine both processed datasets into a single data frame suitable for plotting.
    3.  **Render Comparison Plot**: Using the combined dataset, I'll render a stacked area chart (`geom_area`) that shows the production breakdown by product type over the years. The chart should be faceted by the data source (`SQL` vs. `API`) to allow for a clear side-by-side comparison.

* I'm providing the complete, updated `server` function. I need to replace the existing `server` function in the R script with this new version. This new function includes all the necessary `observeEvent`, `eventReactive`, and `renderPlot` calls to implement the API integration and comparison chart as described above. The
