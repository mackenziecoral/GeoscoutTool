...
server <- function(input, output, session) {
  ...
  observeEvent(input$toggle_api_comparison, {
    ...
    if (input$toggle_api_comparison == TRUE && is.null(reactive_vals$api_base_data)) {
      ...
      base_url <- "https://data.woodmac.com/query/upstream_weekly/imp-met/odata/field_company_net_annual_production_kbd"
      query_params <- list(
        query_params <- list(
          `$filter` = "country_name eq 'Canada' and year ge 2015 and year le 2040",
          `$select` = "company_name,liquid_gas_breakdown,liquid_gas,year,field_name,metric_value,unit"
        )
      ...
      tryCatch({
        ...
        if (httr::status_code(response) == 200) {
          ...
          error_msg <- paste("API Error:", httr::status_code(response), "-", httr::content(response, "text", encoding="UTF-8"))
  ...
  comparison_data <- eventReactive(input$fetch_api_data, {
    req(input$toggle_api_comparison == TRUE, reactive_vals$api_base_data)
    showNotification("Processing comparison data...", type = "message", id = "compProcMsg", duration = NULL)
    
    # 1. Process API Data
    api_data_processed <- data.table()
    if (!is.null(input$api_company_filter) && !is.null(input$api_asset_filter)) {
      ...
      if (nrow(filtered_api) > 0) {
        # Filter for the correct unit and aggregate
    ...
    sql_data <- filtered_group_cumulative_data()$calendar_cumulative_data
    sql_data_processed <- data.table()
    if (!is.null(sql_data) && nrow(sql_data) > 0) {
      
      # We need to get product breakdown for SQL data, which requires re-processing `prod_long`
      # from the filtered_group_cumulative_data reactive
      # This is a bit complex as we need to re-run part of that logic
      # Let's re-calculate from the filtered wells
      filtered_wells_for_sql <- reactive_vals$wells_to_display
      if (nrow(filtered_wells_for_sql)>0) {
        # Simplified recalculation for product breakdown
        # This part requires fetching monthly production again.
        # Note: This could be optimized by storing the initial prod_long table
        # For now, we'll re-fetch for simplicity
        target_uwis_sql <- unique(filtered_wells_for_sql$GSL_UWI_Std)
        # This re-fetch is intensive. For this example, we will just sum up the BOE
        # A full implementation would re-run the fetching logic from filtered_group_cumulative_data
        
        sql_agg_by_year <- sql_data[, .(
          TotalBOE_d = sum(AvgDailyBOERate_Cal, na.rm=TRUE) / 1000 # Convert boe/d to kboe/d
        ), by = .(Year = year(PROD_DATE))]
        
        # To match the stacked chart, we'll assign the whole value to a single "Product"
        sql_data_processed <- sql_agg_by_year[, .(
          Year,
          Product = "Total BOE (SQL)",
          Value = TotalBOE_d,
          DataSource = "SQL"
        )]
    ...
    if (nrow(api_data_processed) > 0 || nrow(sql_data_processed) > 0) {
      ...
      removeNotification("compProcMsg")
      showNotification("No data found for the selected API filters.", type = "warning", duration = 4)
      return(NULL)
    }
  })
